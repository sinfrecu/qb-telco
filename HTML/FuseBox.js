...
$(function () {
        ...
        if (Status === true){
            ...
            SwitchStatus.forEach(function (CurrentFuse, CurrentIndex) {
                ...
                if (RandomNumber === 1){
                    ...
                    document.getElementById("Fuse" + (CurrentIndex + 1)).style.backgroundColor = "rgb(0, 0, 0)"
                }
            })
        } else {
            $(".box").hide()
    ...
    document.onkeydown = function (KeyData) {
        if (KeyData.which === 27 || KeyData.which === 18) {
            $.post("https://qb-telco/main", JSON.stringify({ReturnType: "EXIT", ReturnData: "-"}))
        }
    }
    ...
    $(".fusebutton").click(function () {
        ...
        if (SwitchStatus[ClickedFuseArrayId] === false) {
            ...
            document.getElementById(ClickedFuseid).style.backgroundColor = "rgb(0, 0, 0)"
        }
        ...
        if (FoundWrong === false){
            $.post("https://qb-telco/main", JSON.stringify({ReturnType: "DONE", ReturnData: "-"}))
        }
    })
})
